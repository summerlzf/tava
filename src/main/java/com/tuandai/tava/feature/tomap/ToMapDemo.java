...
public class ToMapDemo {
    ...
    public Map<String, UserInfo> new2(List<UserInfo> list) {
        return list.stream().collect(Collectors.toMap(UserInfo::getId, Function.identity()));
    }

    @Test
    public void test() {
        List<UserInfo> list = UserInfoFactory.create();
        Map<String, UserInfo> map = new2(list);
        map.forEach((key, val) -> {
